...
text_clf = Pipeline([('vect', CountVectorizer()),
                    ('tfidf', TfidfTransformer(norm="l2")),('clf',LogisticRegression(solver='lbfgs', penalty='l2',  max_iter= 10, random_state=42))])
...
submission(bayes_clf.predict(data_test),'./Models/naivesBayes.csv')
submission(lr_clf.predict(data_test), './Models/lr.csv')
submission(svc_clf.predict(data_test),'./Models/svm_withoutGloVe.csv')
